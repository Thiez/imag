...
pub mod iter {
    ...
    impl<'a> GetIter<'a> {
        ...
        pub fn without_unlinked<F>(self) -> FilterLinksIter<'a, F>
            where F: FnOnce(&[Link]) -> bool
        {
            FilterLinksIter(self, |links: &[Link]| links.len() > 0)
        }
        ...
        ///
        /// This does _not_ remove the entries from the store.
        pub fn with_less_than_n_links<F>(self, n: usize) -> FilterLinksIter<'a, F>
            where F: FnOnce(&[Link]) -> bool
        {
            FilterLinksIter(self, |links: &[Link]| links.len() < n)
        }
        ...
        ///
        /// This does _not_ remove the entries from the store.
        pub fn with_more_than_n_links<F>(self, n: usize) -> FilterLinksIter<'a, F>
            where F: FnOnce(&[Link]) -> bool
        {
            FilterLinksIter(self, |links: &[Link]| links.len() > n)
        }
        ...
        ///
        /// This does _not_ remove the entries from the store.
        pub fn filtered_for_links<F>(self, f: F) -> FilterLinksIter<'a, F>
            where F: FnOnce(&[Link]) -> bool
        {
            FilterLinksIter(self, f)
        }
    ...
    /// taken.
    pub struct FilterLinksIter<'a, F>(GetIter<'a>, F)
        where F: FnOnce(&[Link]) -> bool;

    impl<'a, F: FnOnce(&[Link]) -> bool> Iterator for FilterLinksIter<'a, F> {
        type Item = Result<FileLockEntry<'a>>;
